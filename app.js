        ...
        await fetch('/api/notify', {
            ...
            body: JSON.stringify({
                embeds: [{
                    ...
                    footer: { text: "Portail Médical RP" }
                }]
            })
        });
    } catch (e) {
        console.error("Erreur notification Discord:", e);
    }
}
...
function setupEventListeners() {
    ...
    document.getElementById('btn-delete-patient').addEventListener('click', async () => {
        if (confirm('Êtes-vous sûr de vouloir supprimer définitivement ce dossier patient ?')) {
            ...
            sendDiscordNotification(
                "🗑️ Dossier Patient Supprimé",
                `Le dossier du patient **${appState.activePatientId}** a été supprimé par **${appState.currentDoctorName}**.`,
                15158588 // Red
            );
    ...
    document.getElementById('form-add-patient').addEventListener('submit', async (e) => {
        ...
        if (appState.editingPatientId) {
            ...
            sendDiscordNotification(
                "📝 Dossier Patient Modifié",
                `Le dossier de **${patientData.first_name} ${patientData.last_name}** (ID: ${appState.editingPatientId}) a été mis à jour par **${appState.currentDoctorName}**.`,
                15844367 // Gold
            );
            ...
            sendDiscordNotification(
                ...
                `Un nouveau dossier a été créé pour **${newPatient.first_name} ${newPatient.last_name}** (ID: ${newPatient.id}) par **${appState.currentDoctorName}**.`,
                3066993 // Green
            );
        }
    ...
    document.getElementById('form-add-intervention').addEventListener('submit', async (e) => {
        ...
        sendDiscordNotification(
            ...
            `**Patient:** ${patient.firstName} ${patient.lastName} (ID: ${patient.id})\n**Type:** ${newIntervention.title}\n**Médecin:** ${newIntervention.doctor_name}\n**Gravité:** ${newIntervention.severity.toUpperCase()}\n\n${newIntervention.description}`,
            newIntervention.severity === 'elevee' ? 15158588 : (newIntervention.severity === 'moyenne' ? 15844367 : 3066993)
        );
    });
...
${obs ? `**Observations :**\n${obs}` : ''}
        ...
        sendDiscordNotification(
            "🩺 Nouvelle Visite Médicale (QCM)",
            `**Patient:** ${patient.firstName} ${patient.lastName} (ID: ${patient.id})\n**Médecin:** ${newIntervention.doctor_name}\n\n${newIntervention.description}`,
            newIntervention.severity === 'elevee' ? 15158588 : 3447003
        );
    });
    ...
    document.getElementById('form-recrutement').addEventListener('submit', async (e) => {
        ...
        sendDiscordNotification(
            "📝 Nouveau Recrutement",
            `**Candidat:** ${newRecrutement.name}\n**Évaluateur:** ${newRecrutement.evaluator}\n**Score:** ${score}/4\n**Décision:** ${decision}\n\n*Notes: ${newRecrutement.notes || 'Aucune'}*`,
            decision === 'Accepté' ? 3066993 : (decision === 'En attente' ? 15844367 : 15158588)
        );
    });
    ...
    document.getElementById('form-ppa').addEventListener('submit', async (e) => {
        ...
        sendDiscordNotification(
            "🔫 Nouveau Test PPA",
            `**Candidat:** ${newPpa.name}\n**Évaluateur:** ${newPpa.evaluator}\n**Score:** ${score}\n**Décision:** ${decision}\n\n*Notes: ${newPpa.notes || 'Aucune'}*`,
            decision === 'Apte' ? 3066993 : 15158588
        );
    });
}
